...
export default class X68000 {
	...
	drawGraphic(data, y) {
		...
		if (this.memmode_v < 3) {
			...
			case 0: // 512x512 16 colors
				if (r2 & 15) {
					...
					if (r2 & 1) {
						if ((r2 & 0x5400) === 0x1400) // Special priority
							for (let px, p = 16, x = 0; x < cx << 1; ++p, x += 2)
					...
					if ((r2 & 0x4400) === 0x4400) // translucent
				...
				if ((r2 & 0x5c00) === 0x1c00 && r2 & 1) // translucent
			...
			case 1: // 512x512 256 colors
				if (r2 & 15) {
					...
					if (r2 & 3) {
						if ((r2 & 0x5400) === 0x1400 && r2 & 3) // Special priority / translucent
							for (let px, p = 16, x = 0; x < cx << 1; ++p, x += 2)
				...
				if ((r2 & 0x5c00) === 0x1c00 && r2 & 3) // translucent
					for (let px, sel, p = 1024 * (y + 16) + 16, x = 0; x < cx << 1; ++p, x += 2) {
				...
				return;
			case 2: // 512x512 65536 colors
				if (!(r2 & 15))
					return;
				if ((r2 & 0x5c00) === 0x1400) // Special priority
				...
				else if ((r2 & 0x5c00) === 0x1c00) // translucent
				...
				else if ((r2 & 0x4400) === 0x4400) // translucent
		...
		} else if (r2 & 16) { // 1024x1024 16 colors
