...
class JumpBug {
	...
	constructor() {
		...
		convertGFX(this.bg.subarray(0x8000), BG.subarray(0x800), 512, rseq(8, 0, 8), seq(8), [0, BG.length * 4], 8);
		convertGFX(this.obj, BG, 64, rseq(8, 128, 8).concat(rseq(8, 0, 8)), seq(8).concat(seq(8, 64)), [0, BG.length * 4], 32);
		convertGFX(this.obj.subarray(0x8000), BG.subarray(0x800), 128, rseq(8, 128, 8).concat(rseq(8, 0, 8)), seq(8).concat(seq(8, 64)), [0, BG.length * 4], 32);
	...
	xfer8x8(data, p, k, i) {
		let q = this.ram[k] << 6, idx = this.ram[0xc01 + i * 2] << 2 & 0x1c;

		if (this.mmo[4] && (this.ram[k] & 0xc0) === 0x80)
			q = (this.ram[k] & 0x3f | this.bank << 6 | 0x200) << 6 & 0xffc0;
	...
	xfer16x16(data, dst, src) {
		...
		if ((dst & 0xff) === 0 || (dst & 0xff) >= 240 || (dst & 0x1ff00) === 0 || dst >= 272 * 0x100)
			return;
		if (this.mmo[4] && (src & 0x30) === 0x20)
			src = src << 8 & 0x0f00 | this.bank << 12 | 0x8000;
		else
			src = src << 8 & 0x3f00;
	...
	xfer16x16V(data, dst, src) {
		...
		if ((dst & 0xff) === 0 || (dst & 0xff) >= 240 || (dst & 0x1ff00) === 0 || dst >= 272 * 0x100)
			return;
		if (this.mmo[4] && (src & 0x30) === 0x20)
			src = (src << 8 & 0x0f00 | this.bank << 12 | 0x8000) + 256 - 16;
		else
	...
	xfer16x16H(data, dst, src) {
		...
		if ((dst & 0xff) === 0 || (dst & 0xff) >= 240 || (dst & 0x1ff00) === 0 || dst >= 272 * 0x100)
			return;
		if (this.mmo[4] && (src & 0x30) === 0x20)
			src = (src << 8 & 0x0f00 | this.bank << 12 | 0x8000) + 16;
		else
			src = (src << 8 & 0x3f00) + 16;
		for (let i = 16; i !== 0; dst += 256 - 16, src += 32, --i)
			for (let j = 16; j !== 0; dst++, --j)
	...
	xfer16x16HV(data, dst, src) {
		...
		if ((dst & 0xff) === 0 || (dst & 0xff) >= 240 || (dst & 0x1ff00) === 0 || dst >= 272 * 0x100)
			return;
		if (this.mmo[4] && (src & 0x30) === 0x20)
			src = (src << 8 & 0x0f00 | this.bank << 12 | 0x8000) + 256;
		else
