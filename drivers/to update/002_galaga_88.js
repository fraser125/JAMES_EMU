...
class Galaga88 {
	...
	constructor() {
		...
		for (let page = 0; page < 0x8000; page++)
			...
			else if (range(page, 0x6800, 0x6fff))
				this.memorymap[page].base = PRG.base[0x200 | page & 0xff];
			else if (range(page, 0x7000, 0x77ff))
				this.memorymap[page].base = PRG.base[0x300 | page & 0xff];
			else if (range(page, 0x7800, 0x7fff))
				this.memorymap[page].base = PRG.base[0x400 | page & 0xff];
		...
		//SETUP VIDEO
		this.chr.set(CHR);
		convertGFX(this.obj, OBJ, 1536, rseq(16, 2048, 64).concat(rseq(16, 0, 64)), seq(16, 0, 4).concat(seq(16, 1024, 4)), seq(4), 512);
...
import {ROM} from "../roms/galaga_88.png.js";
