...
class FantasyZone {
	...
	updateStatus() {
		...
		if (this.fDIPSwitchChanged) {
			...
			switch (this.nRank) {
			case 'Easy':
				this.in[5] = this.in[5] & ~0xc0 | 0x80;
				break;
			case 'Normal':
				this.in[5] |= 0xc0;
				break;
			case 'Hard':
				this.in[5] = this.in[5] & ~0xc0 | 0x40;
				break;
			case 'Hardest':
				this.in[5] &= ~0xc0;
				break;
	...
	drawObj(data, cat) {
		for (let k = 0x11000; k < 0x11800; k += 0x10) {
			...
			const color = this.ram[k + 8] << 4 & 0x3f0 | 0x400, obank = this.ram[k + 9] << 12 & 0x30000;
...
import {ROM} from "../roms/fantasy_zone.png.js";
