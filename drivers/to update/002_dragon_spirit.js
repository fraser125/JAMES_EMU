...
class DragonSpirit {
	...
	in = Uint8Array.of(0xff, 0xff, 0x96, 0xf8);
...
//			this.ram4[0x14] = 0; //for cold boot
	...
	updateInput() {
		...
		this.fTurboA && (this.in[0] ^= 1 << 4);
		this.fTurboB && (this.in[0] ^= 1 << 5);
		return this;
	}
	...
	triggerX(fDown) {
		!(this.fTurboA = fDown) && (this.in[0] |= 1 << 4);
	}

	triggerY(fDown) {
		!(this.fTurboB = fDown) && (this.in[0] |= 1 << 5);
	}
...
import {ROM} from "../roms/dragon_spirit.png.js";
