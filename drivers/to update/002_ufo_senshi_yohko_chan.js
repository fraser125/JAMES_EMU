...
import MC8123 from '../libs/EMU.js/devices/CPU/mc8123.js';
import Z80 from '../libs/EMU.js/devices/CPU/z80.js';
let game, sound;

class UfoSenshiYohkoChan {
	cxScreen = 224;
	cyScreen = 256;
	width = 256;
	height = 512;
	...
	in = Uint8Array.of(0xff, 0xff, 0xff, 0x8d, 0xff);
	bank = 0x80;
	cpu2_nmi = false;
	cpu2_irq = false;
	...
	makeBitmap(flag) {
		...
		//bg drawing
		for (let i = 0; i < 64; i++) {
			const hScroll = this.ram[0x27ba], vScroll = this.ram[0x27c0 | i << 1 & 0x3e] | this.ram[0x27c1 | i << 1 & 0x3e] << 8 & 0x100;
			for (let j = 0; j < 64; j++) {
...
import {ROM} from "../roms/ufo_senshi_yohko_chan.png.js";
