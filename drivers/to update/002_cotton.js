...
import UPD7759 from '../libs/EMU.js/devices/SOUND/upd7759.js';
...
import FD1094 from '../libs/EMU.js/devices/CPU/fd1094.js';
...
class Cotton {
	...
	constructor() {
		...
		for (let page = 0; page < 0x2000; page++)
			if (range(page, 0, 0xff, 0xff00))
				this.memorymapper.memorymap[2][page].write = (addr, data) => { this.bgbank[addr & 1] = data << 12 & 0x7000; };
		for (let page = 0; page < 0x2000; page++)
		...
		this.isspace = Uint8Array.from(seq(32768), i => Number(this.bg.subarray(i << 6, i + 1 << 6).every(e => !e)));
	}
	...
	updateStatus() {
		...
		if (this.fDIPSwitchChanged) {
			...
			switch (this.nLife) {
			case 2:
				this.in[4] = this.in[4] & ~6 | 4;
				break;
			case 3:
				this.in[4] |= 6;
				break;
			case 4:
				this.in[4] = this.in[4] & ~6 | 2;
				break;
			case 5:
				this.in[4] &= ~6;
				break;
...
import {ROM} from "../roms/cotton.png.js";
