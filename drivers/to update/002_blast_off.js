...
class BlastOff {
	...
	constructor() {
		...
		for (let page = 0; page < 0x8000; page++)
			if (range(page, 0x2e00, 0x2e17)) {
				...
				this.memorymap[page].read = (addr) => {
					return [183, 0, 0, 0, 0, this.key[3] << 4 & 0xf0 | this.key[3] >> 4, 0, Math.floor(Math.random() * 256)][addr >> 4 & 7];
				};
				...
				this.memorymap[page].base = PRG.base[page & 0x1ff];
			else if (range(page, 0x7800, 0x7fff))
				this.memorymap[page].base = PRG.base[0x200 | page & 0x1ff ^ 0x100];
		...
		//SETUP VIDEO
		this.chr.set(CHR);
		convertGFX(this.obj, OBJ, 1280, rseq(16, 2048, 64).concat(rseq(16, 0, 64)), seq(16, 0, 4).concat(seq(16, 1024, 4)), seq(4), 512);
...
import {ROM} from "../roms/blast_off.png.js";
