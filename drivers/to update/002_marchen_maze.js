...
import YM2151 from '../libs/EMU.js/devices/SOUND/ym2151.js';
import C30 from '../libs/EMU.js/devices/SOUND/c30.js';
import {seq, rseq, convertGFX, Timer} from '../libs/EMU.js/utils.js';
import {init} from '../libs/EMU.js/main.js';
import {dummypage} from '../libs/EMU.js/devices/CPU/cpu.js';
import MC6809 from '../libs/EMU.js/devices/CPU/mc6809.js';
import MC6801 from '../libs/EMU.js/devices/CPU/mc6801.js';
let game, sound;
...
class MarchenMaze {
	...
	constructor() {
		...
		for (let page = 0; page < 0x8000; page++)
			if (range(page, 0x2e00, 0x2e17)) {
				...
				this.memorymap[page].read = (addr) => {
					...
					if ((addr & 0x1fff) < 5)
						return d ? [n % d >> 8, n % d & 0xff, n / d >> 8 & 0xff, n / d & 0xff, 0x25][addr & 7] : [0, 0, 0xff, 0xff, 0x25][addr & 7];
					return 0;
				};
				this.memorymap[page].write = (addr, data) => { (addr & 0x1fff) < 4 && (this.key[addr & 3] = data); };
				...
				this.memorymap[page].write = null;
			} else if (range(page, 0x4000, 0x57ff))
				this.memorymap[page].base = PRG.base[page >> 2 & 0x600 | page & 0x1ff];
			else if (range(page, 0x7000, 0x7fff))
				this.memorymap[page].base = PRG.base[0x600 | page >> 3 & 0x100 | page & 0xff];
		...
		//SETUP VIDEO
		this.chr.set(CHR);
		convertGFX(this.obj, OBJ, 1024, rseq(16, 2048, 64).concat(rseq(16, 0, 64)), seq(16, 0, 4).concat(seq(16, 1024, 4)), seq(4), 512);
...
import {ROM} from "../roms/marchen_maze.png.js";
...
window.addEventListener('load', () => expand(ROM).then(ROM => {
	...
	VOI = new Uint8Array(ROM.buffer, 0xa1000, 0x40000).addBase();
	CHR8 = new Uint8Array(ROM.buffer, 0xe1000, 0x20000);
	CHR = new Uint8Array(ROM.buffer, 0x101000, 0xc0000);
	OBJ = new Uint8Array(ROM.buffer, 0x1c1000, 0x80000);
	NVRAM = new Uint8Array(ROM.buffer, 0x241000, 0x800);
