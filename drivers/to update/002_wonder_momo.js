...
import Namco63701X from '../libs/EMU.js/devices/SOUND/namco_63701x.js';
...
class WonderMomo {
	...
	obj = new Uint8Array(0x100000).fill(15);
...
//			this.cpu.memorymap[0x88 + i].write = (addr) => { this.bgbank = addr >> 10 & 1; };
		...
		for (let i = 0; i < 0x20; i++) {
			...
			this.cpu2.memorymap[0x20 + i].write = null;
		}
		for (let i = 0; i < 0x40; i++) {
			this.cpu2.memorymap[0x40 + i].base = this.ram.base[i];
		...
		this.mcu.memorymap[0x38].read = (addr) => {
			switch (addr & 0xff) {
			case 1:
		...
		//SETUP VIDEO
		convertGFX(this.bg1, BG1, 2048, rseq(8, 0, 8), seq(8), [Math.floor(BG1.length / 3) * 16, -1, -1], 8);
		convertGFX(this.bg1, BG1, 2048, rseq(8, 0, 16), seq(4).concat(seq(4, 8)), [0, 4], 16);
		convertGFX(this.bg2, BG2, 2048, rseq(8, 0, 8), seq(8), [Math.floor(BG2.length / 3) * 16, -1, -1], 8);
		convertGFX(this.bg2, BG2, 2048, rseq(8, 0, 16), seq(4).concat(seq(4, 8)), [0, 4], 16);
	...
	makeBitmap(flag) {
		...
		for (let pri = 0; pri < 8; pri++) {
			...
			for (let k = 0x5800, i = 127; i !== 0; k += 16, --i) {
				...
				const src = (~ram[k + 8] & 0x18 | 7) & -w | (ram[k + 4] & -h) << 5 & 0x300 | ram[k + 5] << 10 & 0x1fc00 | ram[k + 4] << 17 & 0xe0000;
...
import {ROM} from "../roms/wonder_momo.png.js";
