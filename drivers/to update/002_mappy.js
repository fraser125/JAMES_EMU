...
class Mappy {
	...
	updateStatus() {
		...
		if (this.fDIPSwitchChanged) {
			...
			switch (this.nBonus) {
			case 'A':
				this.in[5] &= ~8, this.in[9] &= ~3;
				break;
			case 'B':
				this.in[5] |= 8, this.in[9] &= ~3;
				break;
			case 'C':
				this.in[5] &= ~8, this.in[9] = this.in[9] & ~3 | 1;
				break;
			case 'D':
				this.in[5] |= 8, this.in[9] = this.in[9] & ~3 | 1;
				break;
			case 'E':
				this.in[5] &= ~8, this.in[9] = this.in[9] & ~3 | 2;
				break;
			case 'F':
				this.in[5] |= 8, this.in[9] = this.in[9] & ~3 | 2;
				break;
...
//				this.in[7] &= ~4;
	...
	updateInput() {
		...
		if (this.port[8] === 1)
			...
			this.port.set([8, 4, 6, 0xe, 0xd, 9, 0xd], 1);
		if (this.port[0x18] === 4)
			...
			this.port.set([8, 4, 6, 0xe, 0xd, 9, 0xd], 0x11);
		return this.edge = this.in[3] ^ 0xf, this;
	}
...
import {ROM} from "../roms/mappy.png.js";
