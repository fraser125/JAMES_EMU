...
const RomSetInfo = [
	{
		// Mame name  'zigzagb'
		display_name: 'Zig Zag (bootleg Dig Dug conversion on Galaxian hardware, set 1)',
		developer: 'bootleg (LAX)',
		year: '1982',
		Notes: '~AUTO PORTED PLEASE TEST~',

		archive_name: 'zigzagb',
		driver: ZigZag,
		mappings: [
		{
			name: 'RGB',
		...
		{
			name: 'PRG',
			roms: ['zz_d1.7l', 'zz_d2.7k', 'zz_d4.7f', 'zz_d3.7h'],
		},
		{
			name: 'OBJ',
			roms: ['zz_6.1h'],
	...
	{
		// Mame name  'zigzagb2'
		display_name: 'Zig Zag (bootleg Dig Dug conversion on Galaxian hardware, set 2)',
		developer: 'bootleg (LAX)',
		year: '1982',
		Notes: '~AUTO PORTED PLEASE TEST~',
		...
		{
			name: 'PRG',
			roms: ['z1.7l', 'zz_d2.7k', 'zz_d4.7f', 'zz_d3.7h'],
		},
		{
			name: 'OBJ',
			roms: ['zz_6.1h'],
...
window.addEventListener('load', () =>
	RBL.Load_Rom(RomSetInfo[ROM_INDEX]).then((ROM) => {
		...
		PRG   = ROM["PRG"].addBase();
		BG    = cat(ROM["OBJ"].subarray(0, 0x800), ROM["BG"].subarray(0, 0x800)); // the two roms have to be swapped like this.
		OBJ   = cat(ROM["OBJ"].subarray(   0x800), ROM["BG"].subarray(   0x800)); // ^
		RGB   = ROM["RGB"];
